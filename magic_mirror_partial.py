...
def page_emotion():
    ...
    x = st.slider("자기표현 정도 (X축 1=내향 9=외향)", 1, 9,
                  st.session_state.emotion["x"])
    y = st.slider("감정 방향성 (Y축 1=이성 9=감성)", 1, 9,
                  st.session_state.emotion["y"])
...
def page_reco():
    ...
    st.write(f"**감정 좌표**: ({st.session_state.emotion['x']}, "
             f"{st.session_state.emotion['y']})")
